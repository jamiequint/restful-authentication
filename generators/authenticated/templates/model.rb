...
<% if options[:include_activation] && !options[:stateful] %>
  ...
  def self.authenticate(email, password)
    ...
    u = <% if    options[:stateful]           %>find_in_state :first, :active, :conditions => {:login => email.downcase}<%
           elsif options[:include_activation] %>find :first, :conditions => ['email = ? and activated_at IS NOT NULL', login]<%
